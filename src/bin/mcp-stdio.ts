...
const network = (process.env.RIPAR_NETWORK as "testnet" | "mainnet" | undefined) ?? "testnet";
...
async function main(): Promise<void> {
  ...
  console.error(
    `ripar-skills MCP server on stdio (${network}, identity app ${REGISTRY_APP_IDS.testnet.identity})`
  );
}
