...
export function jobStatusName(code) {
    const k = Number(code);
...
export function explorerTxUrl(cfg, txId) {
    return `${cfg.explorer}/tx/${txId}`;
}
export function explorerAppUrl(cfg, appId) {
    return `${cfg.explorer}/application/${appId}`;
}
export function explorerAddressUrl(cfg, address) {
