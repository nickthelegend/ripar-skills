...
if (!payTo) {
    console.error("RIPAR_PAY_TO is required: the card quotes prices, and a price with no payee is not a quote.");
    process.exit(1);
}
if (!agentId) {
    console.warn("RIPAR_AGENT_ID is 0, so this card makes no on-chain claim. Peers that verify it will say so.");
}
const card = riparAgentCard({
    name: process.env.RIPAR_AGENT_NAME ?? "Ripar Skills Agent",
    description: process.env.RIPAR_AGENT_DESCRIPTION ??
    ...
    agentId,
    payTo,
    network: process.env.RIPAR_NETWORK ?? "testnet",
