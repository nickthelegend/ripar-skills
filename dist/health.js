 ...
 * itself.
 */
export async function agentHealth(config, input, opts = {}) {
    ...
    try {
        ...
        cardError = err instanceof AgentCardError ? describeCardError(err) : err.message;
        checks.push({
            id: "card_reachable",
            title: "Serves an A2A agent card",
            status: "fail",
            detail: `No usable agent card at ${origin}. ${cardError} ` +
...
async function resolveAgent(registry, input) {
    ...
    if (input.domain) {
        ...
        if (id === 0) {
            throw new Error(`${input.domain} is not registered in the IdentityRegistry. There is no on-chain record to ` +
                `check a card against, and grading a self-published document against itself proves nothing.`);
        }
        const agent = await registry.getAgent(id);
        if (!agent)
            throw new Error(`Domain ${input.domain} resolves to agent ${id}, which has no record`);
        return agent;
    }
    if (input.address) {
        const id = await registry.resolveByAddress(input.address);
        if (id === 0)
            throw new Error(`${input.address} controls no agent in the IdentityRegistry`);
        const agent = await registry.getAgent(id);
        if (!agent)
            throw new Error(`Address ${input.address} resolves to agent ${id}, which has no record`);
        return agent;
    }
    throw new Error("agentHealth needs one of agentId, domain, or address");
}
function describeCardError(err) {
    const detail = err.details;
    return detail?.length ? `${err.message}: ${detail.join("; ")}` : err.message;
}
async function checkHealthEndpoint(doFetch, origin, paths, timeoutMs) {
    const tried = [];
    for (const path of paths) {
        ...
        try {
            ...
            tried.push({ url, status: `${err.name}: ${err.message}` });
        }
    }
    ...
    return {
        ...
        title: "Answers a liveness probe",
        status: "unknown",
        detail: `Nothing answered at ${paths.join(" or ")}. That is not proof the agent is down — a health ` +
...
function payToCheck(agent, card) {
    ...
    if (!payTo) {
        return {
            ...
            title,
            status: "skip",
            detail: "The card carries no x402 pricing extension, so it names no payment account. Nothing to " +
    ...
    if (payTo === agent.address) {
        return {
            ...
            title,
            status: "pass",
            detail: `The card asks for payment at ${payTo}, which is the address the IdentityRegistry holds for ` +
    ...
    return {
        ...
        title,
        status: "fail",
        detail: `THE CARD ASKS YOU TO PAY A DIFFERENT ACCOUNT. It names ${payTo}; the registry holds ` +
...
async function agentIdCheck(registry, agent, card, origin) {
    ...
    if (!claimed) {
        return {
            ...
            title,
            status: "fail",
            detail: `The card at ${origin} makes NO on-chain identity claim — no registry extension, or an ` +
    ...
    if (claimed === agent.agentId) {
        return {
            ...
            title,
            status: "pass",
            detail: `The card claims agent ${claimed}, and agent ${claimed}'s registered domain is ` +
    ...
    return {
        ...
        title,
        status: "fail",
        detail: `The card at ${origin} claims to be agent ${claimed}, but this domain is registered as agent ` +
...
async function serves402Check(doFetch, config, card, timeoutMs) {
    ...
    if (!endpoint || pricedSkills.length === 0) {
        return {
            ...
            title,
            status: "skip",
            detail: "The card prices nothing, so there is no paywall to test. A free agent is not a broken one.",
    ...
    let res;
    try {
        res = await fetchWithTimeout(doFetch, endpoint, {
            method: "POST",
        ...
        return {
            ...
            title,
            status: "unknown",
            detail: `Could not reach the priced endpoint ${endpoint}: ${err.message}. The paywall is ` +
                `UNVERIFIED — this is not a pass, and it is not evidence the endpoint is free either.`,
            evidence: { endpoint, error: err.message },
        };
    }
    ...
    if (res.status !== 402) {
        return {
            ...
            title,
            status: "fail",
            detail: `The card prices ${pricedSkills.map(([k]) => k).join(", ")}, but an UNPAID request to ` +
    ...
    if (!challenge) {
        return {
            ...
            title,
            status: "fail",
            detail: `${endpoint} answered 402 but carried no readable payment requirements — nothing in the ` +
    ...
    const best = [...challenge.accepts].sort((a, b) => Number(a.maxAmountRequired) - Number(b.maxAmountRequired))[0];
    ...
    const rightChain = best.network.startsWith(wantNetwork) || wantNetwork.startsWith(best.network);
    ...
    return {
        ...
        title,
        status: "pass",
        detail: `An unpaid request to ${endpoint} got a real 402 (challenge read from the ${challenge.from}) ` +
...
function verdictOf(checks, agent, config) {
    ...
    const nothingAnswered = cardCheck.status === "fail" &&
        checks.every((c) => c.status !== "pass");
    const verdict = nothingAnswered
        ? "unreachable"
        : failed.length
            ? "failing"
            : unknown.length
    ...
    const summary = verdict === "unreachable"
...
function truncate(value) {
    ...
    if (text && text.length > 800)
        return `${text.slice(0, 800)}… (${text.length} bytes)`;
    return value;
}
