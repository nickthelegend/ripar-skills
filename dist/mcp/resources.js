...
function idFromUri(uri, variable, kind) {
...
export function registerRiparResources(server, ctx) {
    ...
    server.registerResource("ripar-registries", RESOURCE_URIS.registries, {
        title: "Ripar registries",
        description: "The three live registry app ids, the endpoints they are read from, and the escrow terms " +
        ...
        return json(uri, {
            ...
            escrow: {
                ...
                note: "The asset and the window are fixed at bootstrap, not per job — an escrow whose asset " +
    ...
    // --- The roster.
    server.registerResource("ripar-agents", RESOURCE_URIS.agents, {
        title: "Registered agents",
        description: "Every agent in the IdentityRegistry: id, domain, controlling address, and where its A2A " +
    ...
    server.registerResource("ripar-agent", new ResourceTemplate(RESOURCE_URIS.agent, {
        ...
        title: "One agent",
        description: "An agent's registry record, its reputation score, and the jobs it is serving or " +
    ...
    server.registerResource("ripar-jobs", RESOURCE_URIS.jobs, {
        title: "Jobs, with budget and escrow",
        description: "Every job on the ValidationRegistry with BOTH money numbers: budget, what the client says " +
    ...
    // --- One job.
    server.registerResource("ripar-job", new ResourceTemplate(RESOURCE_URIS.job, {
        list: async () => {
        ...
        title: "One job",
        description: "A single job: its lifecycle state, the agents serving and validating it, the spec and " +
